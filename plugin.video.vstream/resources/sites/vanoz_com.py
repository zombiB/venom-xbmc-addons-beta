...
SITE_IDENTIFIER = 'vanoz_com' #identifant nom de votre fichier remplacer les espaces et les . par _ aucun caractere speciale
SITE_NAME = 'Vanoz.com' # nom que xbmc affiche
SITE_DESC = 'Film en streaming' #description courte de votre source

URL_MAIN = 'http://www.vanoz.com' # url de votre source

#definis les url pour les catégories principale ceci et automatique si la deffition et présente elle seras afficher.

MOVIE_NEWS = ('http://www.vanoz.com/twona/', 'showMovies')

MOVIE_GENRES = (True, 'showGenre') 

URL_SEARCH = ('http://www.vanoz.com/twona/index.php?ordering=&searchphrase=all&Itemid=1&option=com_search&searchword=', 'showMovies')
...
def showSearch():
    ...
    if (sSearchText != False):
            sUrl = 'http://www.vanoz.com/twona/index.php?ordering=&searchphrase=all&Itemid=1&option=com_search&searchword=' + sSearchText
            showMovies(sUrl)
...
def showGenre(): #affiche les genres
    ...
    liste = []
    liste.append( ['Action','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=1:action-&Itemid=6&layout=default'] )
    liste.append( ['Aventure','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=4:aventure-&Itemid=6&layout=default'] )
    liste.append( ['Comedie','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=6:comedie-&Itemid=6&layout=default'] )
    liste.append( ['Drame','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=7:drame-&Itemid=6&layout=default'] )
    liste.append( ['Epouvante Horreur','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=9:horreur-&Itemid=6&layout=default'] ) 
    liste.append( ['Fantastique','http://www.vanoz.com/twona/index.php?option=com_content&view=category&id=8:fantastique-&Itemid=6&layout=default'] )  
...
def showMovies(sSearch = ''):
    ...
    if sSearch :
        ...
        if (sUrl == 'http://www.vanoz.com/twona/') :
            sPattern = '<a style="font-size:15px" href="(.+?)" class="latestnews">(\t|\n)+(.+?)<.a><.b>'
        else:
    ...
    if not (aResult == False):
        ...
        for aEntry in aResult:
            ...
            if sSearch:
                sTitle2 = aEntry[4]
                sUrl2 = aEntry[2]
            elif sUrl == 'http://www.vanoz.com/twona/' :
                sTitle2 = aEntry[2]
                sUrl2 = aEntry[0]
            else :
                sTitle2 = aEntry[3]
...
def __checkForNextPage(sHtmlContent): #cherche la page suivante
    sPattern = '<span class="pagenav">[0-9]+<.span><.li><li><a title=".+?" href="(.+?)" class="pagenav">'
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
    if (aResult[0] == True):
...
def showHosters():
    ...
    sPattern = '>>> histoire <<<(.+?)center;">\r+(.+?)\r+<.p>(.+?)>>> Illimité <<<'
    aResult = re.findall(sPattern, sHtmlContent)
    if aResult:
        sComm = unescape(aResult[0][1])
    
    sHtmlContent = sHtmlContent.replace('\r','')
    sPattern = 'class="jwts_tabbertab" title="(.+?)">.+?<iframe src="(\/twona\/.+?)" width='
    aResult = re.findall(sPattern, sHtmlContent)

    #Si il n'a pas de selection de qualitéé
    if (aResult == []):
        sPattern = '<iframe src="(\/twona\/.+?)" width='
        aResult = re.findall(sPattern, sHtmlContent)
        aResult = [('???',aResult[0])]
